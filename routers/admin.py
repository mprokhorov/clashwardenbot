...
async def link_select_player(dm: DatabaseManager,
    ...
    rows = await dm.req_connection.fetch('''
        ...
        WHERE
            ...
            AND ((clan_tag, player_tag) NOT IN (SELECT clan_tag, player_tag FROM master.player_tg_user) 
            OR $2)
...
async def link_finish(dm: DatabaseManager,
    ...
    if len(rows) == 0:
        await dm.req_connection.execute('''
            INSERT INTO master.player_tg_user (clan_tag, player_tag, chat_id, user_id)
            VALUES ($1, $2, $3, $4)
        ''', dm.clan_tag, callback_data.player_tag, callback_data.chat_id, callback_data.user_id)
    ...
    await dm.req_connection.execute('''
        INSERT INTO master.admin_action (chat_id, user_id, action_timestamp, action_description)
...
async def ping_all(dm: DatabaseManager,
    ...
    rows = await dm.req_connection.fetch('''
        ...
        FROM master.chat
        WHERE chat_type IN ('group', 'supergroup') AND chat_id IN (SELECT chat_id FROM master.clan_chat)
    ''')
